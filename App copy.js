...
import { AppContext } from "./components/GameContextWrapper";

export default function App() {
	const [username, setUsername] = useState(null);
	const [showTabBar, setShowTabBar] = useState("none");
	const [nbr_of_dices, NumberOfDices] = useState("none");
	
	return (
		<AppContext.Provider value={{ username, setUsername, setShowTabBar, numberOfDices }}>
			<NavigationContainer>
				<Tab.Navigator
					screenOptions={({ route }) => ({
						headerStyle: {
